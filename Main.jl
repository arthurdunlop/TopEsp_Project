...
print(ufv_valuation_result)

#Atribui valor para a taxa de variação da distrubuição uniforme
variation_Rate = 0.1
#Simulação da geração a partir da geração mensal prevista pelo software PVSyst como média para distrubuição uniforme
uniform_Generation = GenerateUniformGeneration(g, variation_Rate)
#Calcula o VPL a partir da geração uniforme simulada
ufv_valuation_result_Uniform_Generation = Calculate_UFV_Valuation(uniform_Generation, mensal_capital_rate)

print(ufv_valuation_result)
print(ufv_valuation_result_Uniform_Generation)
...
#Grafico do vectors
plot(interval_vector, xlabel="Índice", ylabel="% Entrada no Investimento", title="Entrada no Investimento")
...
heatmap(matrix_from_multiplication, xlabel="% Entrada no Investimento", ylabel="VPL", title="Heatmap da Matriz Resultante")
...
bar(matrix_from_multiplication, xlabel="Linha", ylabel="Valor Médio", title="Médias por Linha")
...
boxplot(matrix_from_multiplication, xlabel="% Entrada no Investimento", ylabel="VPL", title="Boxplot dos Valores na Matriz")

# Violin plot
violin(matrix_from_multiplication', xlabel="% Entrada no Investimento", ylabel="Valor", title="Violin Plot dos Valores na Matriz")