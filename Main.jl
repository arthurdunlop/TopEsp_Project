...
fees_limits = [1000000., 2500000, 3500000]
fee_rates = 1 .+[Convert_Rates_From_aa_To_am(0.04),
Convert_Rates_From_aa_To_am(0.05),
Convert_Rates_From_aa_To_am(0.07),
Convert_Rates_From_aa_To_am(0.08)]

Calculate_Debt_Amount(fee_rates, fees_limits, 500.)
...
plot(mean_of_lowest_values, label="CVar", xlabel="% Capital Próprio no investimento", ylabel="Valor Presente Líquido", title="VPL / % Capital Próprio no investimento")
maximum(mean_of_lowest_values)
max_value_cvar_index = argmax(mean_of_lowest_values)
entry_max_value_cvar = entriesIntoInvestment[max_value_cvar_index]

# Gráfico de linhas das médias por coluna
mean_values = mean(matriz, dims=1)[:]  # médias por coluna
plot!(mean_values, label="Média dos VPLs", xlabel="% Capital Próprio no investimento", ylabel="Valor Presente Líquido", title="VPL / % Capital Próprio no investimento")
maximum(mean_values)
max_value_mean_index = argmax(mean_values)
entry_max_value_mean = entriesIntoInvestment[max_value_mean_index]

# Gráfico de retorno pelo risco
scatter((mean_of_lowest_values, mean_values), xlabel="Retorno", ylabel="Risco", title="Retorno X Risco")
